...
2.图片中表情的协调/不协调性等级
请忽略上面部与下面部之间的空白分割线以及图片的灰度分布不均匀和拼图问题等情况，将上、下面部结合起来视为完整的人脸，回答接下来的问题。你认为图片中表情的协调性等级是多少，请用1—6之间的数字来进行回答（例如，在上一个选择中你的答案为协调，则1代表"些许协调"，2代表"较为协调"，3代表"协调"，4代表"非常协调"，5代表"十分协调", 6代表"极其协调"；不协调如上）
回答：[1/2/3/4/5/6]
...
def main():
    ...
    output_dir = os.path.join(script_dir, "results_qwen_2")
