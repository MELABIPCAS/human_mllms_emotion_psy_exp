...
class EmotionAnalyzer:
    ...
    def parse_response(self, response: str) -> Dict:
        ...
        for answer in self.valid_answers["is_coordinated"]:
            if f"回答：{answer}" in response:
                result["is_coordinated"] = answer
                break

    # 提取协调性等级
        for answer in self.valid_answers["coordination_level"]:
            if f"回答：{answer}" in response:
...
1.图片中所表现出来的表情是否协调
请忽略上面部与下面部之间的空白分割线以及图片的灰度分布不均匀和拼图问题等情况，将上、下面部结合起来视为完整的人脸，回答接下来的问题。基于六种基本情绪来判断此人脸所做出的面部表情是否协调，请用是否来进行回答（如无法进行判断，请说出一个你认为可能性较大的答案）
回答：[是/否]

2.图片中表情的协调/不协调性等级
请忽略上面部与下面部之间的空白分割线以及图片的灰度分布不均匀和拼图问题等情况，将上、下面部结合起来视为完整的人脸，回答接下来的问题。你认为图片中表情的协调性等级是多少，请用1—5之间的数字来进行回答（例如，在上一个选择中你的答案为协调，则1代表"些许协调"，2代表"较为协调"，3代表"协调"，4代表"非常协调"，5代表"十分协调"；不协调如上）
回答：[1/2/3/4/5]

3.图片中的整体情绪类型
请忽略上面部与下面部之间的空白分割线以及图片的灰度分布不均匀和拼图问题等情况，将上、下面部结合起来视为完整的人脸，回答接下来的问题。你认为图片中的表情属于积极情绪还是消极情绪，请用积极情绪或消极情绪来进行回答（如无法进行判断，请说出一个你认为可能性较大的答案）
回答：[积极情绪/消极情绪]

4.图片所表现的情绪
请忽略上面部与下面部之间的空白分割线以及图片的灰度分布不均匀和拼图问题等情况，将上、下面部结合起来视为完整的人脸，回答接下来的问题。你认为图片中的情绪属于高兴、惊讶、伤心、厌恶、害怕、愤怒中的哪一个？请用以上六个词中的一个词来进行回答（如无法进行判断，请说出一个你认为可能性较大的答案）
回答：[高兴/惊讶/伤心/厌恶/害怕/愤怒]

5.判断依据
请忽略上面部与下面部之间的空白分割线以及图片的灰度分布不均匀和拼图问题等情况，将上、下面部结合起来视为完整的人脸，回答接下来的问题。你在判断图片所表达的情绪时，是通过面部哪个区域进行判断的？请用上面部或下面部来进行回答（如无法进行判断，请说出一个你认为可能性较大的答案）
回答：[上面部/下面部]
...
def main():
    ...
    # 修改为Mac路径格式
    image_folder = os.path.expanduser("/home/majc_docker/image_block")  # 请替换成你的图像文件夹路径
    image_paths = analyzer.find_image_paths(image_folder)
    print(f"找到 {len(image_paths)} 个图像待处理")

    # 指定输出目录
    script_dir = os.path.dirname(os.path.abspath(__file__))
    output_dir = os.path.join(script_dir, "results_Gemma_1")
    os.makedirs(output_dir, exist_ok=True)

    # 循环生成30个结果文档
    for run_idx in range(1, 42):
        print(f"\n开始第 {run_idx} 轮处理...")
        ...
        if results:
            print(f"第 {run_idx} 轮: 已处理完 {len(results)} 个图像")

            # 创建带有序号的文件名
            output_file = os.path.join(output_dir, f"gemma_analysis_resultsI{run_idx:02d}.txt")
            with open(output_file, "w", encoding="utf-8") as f:
                for result in results:
                    ...
                    f.write("-" * 80 + "\n")
            print(f"第 {run_idx} 轮结果已保存到文件: {output_file}")
            ...
            excel_output_file = os.path.join(output_dir, f"gemma_analysis_resultsI{run_idx:02d}.xlsx")
            df.to_excel(excel_output_file, index=False)
            print(f"第 {run_idx} 轮结果已保存到Excel文件: {excel_output_file}")
        else:
            print(f"第 {run_idx} 轮: 没有成功处理任何图像")

    print(f"\n所有30轮处理已完成。所有结果文件保存在目录: {output_dir}")
