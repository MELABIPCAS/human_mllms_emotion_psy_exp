...
class EmotionAnalyzer:
    def __init__(self, num_workers=4):
        ...
        # 设备检测
        # 修改设备检测逻辑，避免使用不支持的MPS
        if torch.cuda.is_available():
            self.device = "cuda"
        else:
            self.device = "cpu"  # 使用CPU代替MPS
        ...
        for i in range(1, 4):
            # 修改为适用于当前环境的路径格式
            image_path = f"/home/majc_docker/Image differentiation/coord_example{i}.png"
        ...
        for i in range(1, 4):
            # 使用绝对路径
            image_path = f"/home/majc_docker/Image differentiation/non_coord_example{i}.png"
...
2.图片中表情的协调/不协调性等级
请忽略上面部与下面部之间的空白分割线以及图片的灰度分布不均匀和拼图问题等情况，将上、下面部结合起来视为完整的人脸，回答接下来的问题。你认为图片中人脸面部表情的协调性等级是多少，请用1—5之间的数字来进行回答（例如，在上一个选择中你的答案为协调，则1代表“些许协调”，2代表“较为协调”，3代表“协调”，4代表“非常协调”，5代表“十分协调”；不协调如上）
回答：[1/2/3/4/5]

3.图片中的整体情绪类型
请忽略上面部与下面部之间的空白分割线以及图片的灰度分布不均匀和拼图问题等情况，将上、下面部结合起来视为完整的人脸，回答接下来的问题。你认为图片中的人脸的面部表情属于积极情绪还是消极情绪，请用积极情绪或消极情绪来进行回答（如无法进行判断，请说出一个你认为可能性较大的答案）
回答：[积极情绪/消极情绪]

4.图片所表现的情绪
请忽略上面部与下面部之间的空白分割线以及图片的灰度分布不均匀和拼图问题等情况，将上、下面部结合起来视为完整的人脸，回答接下来的问题。你认为图片中的人脸的表情所表达的情绪属于高兴、惊讶、伤心、厌恶、害怕、愤怒中的哪一个？请用以上六个词中的一个词来进行回答（如无法进行判断，请说出一个你认为可能性较大的答案）
回答：[高兴/惊讶/伤心/厌恶/害怕/愤怒]

5.判断依据
请忽略上面部与下面部之间的空白分割线以及图片的灰度分布不均匀和拼图问题等情况，将上、下面部结合起来视为完整的人脸，回答接下来的问题。你在判断图片中的人脸所表达的情绪时，是通过面部哪个区域进行判断的？请用上面部或下面部来进行回答（如无法进行判断，请说出一个你认为可能性较大的答案）
回答：[上面部/下面部]

注意：请确保完整回答所有5个问题，每个问题都必须有一个明确的答案。请不要解释或讨论，只需要提供简洁明了的答案。
"""
        max_retries = 5
...
def main():
    ...
    # 指定图像文件夹路径
    image_folder = "/home/majc_docker/image_block"  # 使用绝对路径
